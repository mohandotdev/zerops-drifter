  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="shrink-0 pt-1 font-mono text-[0.6875rem] tracking-[0.16em] text-muted-foreground uppercase">
                ...
                <tbody>
                  {Object.entries(finding.details).map(([key, pair]) => (
                    <tr key={key} className="border-b border-border/60 last:border-0">
                      <td className="px-3 py-2 align-top text-muted-foreground">{key}</td>

                      <td className="max-w-[320px] px-3 py-2 align-top break-all text-foreground">
                        {render(pair.staging)}
                      </td>

                      <td className="max-w-[320px] px-3 py-2 align-top break-all text-foreground">
                        {render(pair.production)}
                      </td>
                    </tr>
                  ))}
                </tbody>
...
function ValueBlock({
  ...
  return (
    <div className="min-w-0 rounded-xl border border-border bg-elevated/30 px-3.5 py-3">
      <div className="flex items-center gap-2 font-mono text-[0.6875rem] tracking-[0.16em] text-muted-foreground uppercase">
        <span
          className={`size-1.5 rounded-full ${
            accent === "staging" ? "bg-staging" : "bg-production"
          }`}
          aria-hidden
        />
        {label}
      ...
      <p
        className={`mt-1 break-all font-mono text-sm ${missing ? "text-high" : "text-foreground"}`}
      >
        {value}
      </p>
    </div>
